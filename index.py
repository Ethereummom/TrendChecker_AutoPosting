...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        # Toolbar - Add File
        openFile = QAction(QIcon('./img/folder.png'), '업로드 할 파일 선택' ,self)
        openFile.setShortcut('Ctrl+O')
        openFile.setStatusTip('Tistory에 업로드할 파일을 선택해 주세요.')
        openFile.triggered.connect(self.showDialog)
        
        #Toolbar - Add Category Buttons
        addTagBtn = QAction(QIcon('./img/tag.png'), '태그 입력', self)
        addTagBtn.setShortcut('Ctrl+T')
        addTagBtn.setStatusTip('Tistory에 업로드할 글의 태그를 추가해주세요.')
        
        #Toolbar - Upload Buttons
